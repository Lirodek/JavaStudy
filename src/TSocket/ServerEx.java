...
public class ServerEx extends Thread {
	...
	private void sendmsg(String msg, String userID) {
		...
		User user = userMap.get(str[1]);
		if (user != null) {
			PrintWriter pw = user.pw;
			pw.println(userID + "님이 다음의 귓속말을 보내셨습니다. :" + str[2]);
			pw.flush();
		}
	...
	class ReceiveThread extends Thread {
		...
		String userId = userMap.keySet().toString();
			...
			while (true && in != null) {
				try {
					...
					if (".quit".equalsIgnoreCase(clientMessage)) {
						...
						break;
					} else if (clientMessage.contains(".to")) {
						System.out.println(userId);
						sendmsg(clientMessage, userId);
						
					} else {
						System.out.println(userId.toString() + ">>> " + clientMessage);
						broadcast(userId.toString() + ">>> " + clientMessage);
					}
